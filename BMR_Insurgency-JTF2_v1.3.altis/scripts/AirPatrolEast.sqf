		...
		_aire1 = [_ins_debug] spawn
		{
			...
			for [{_loop=0}, {_loop<1}, {_loop=_loop}] do
			{
				if ((isNull airhunterE1) || (not(alive airhunterE1))) then
				{
					...
					_maxtype = (count INS_OP4_mod_fixedWing)-1;// count types
					_type = INS_OP4_mod_fixedWing select (round random _maxtype);// select random type
					...
					_veh addeventhandler ["killed","(_this select 0) spawn {[_this] call remove_carcass_fnc}"];
					{_x addEventHandler ["killed", "(_this select 0) spawn {[_this] call remove_carcass_fnc}"]} forEach (units _vehgrp);
					_veh addeventhandler ["HandleDamage", {if (((_this select 4) isKindOf "MissileCore") || ((_this select 4) isKindOf "rocketCore")) then { 1; } else { _this select 2; }; }];// Destroy This Air Vehicle With 1 Missle or Rocket
		...
		_aire2 = [_ins_debug] spawn
		{
			...
			for [{_loop=0}, {_loop<1}, {_loop=_loop}] do
			{
				if ((isNull airhunterE2) || (not(alive airhunterE2))) then
				{
					...
					_vehgrp = _vehicle select 2 ;// group of vehicle					
					if (BTC_p_skill isEqualTo 1) then {[_vehgrp] call BTC_AI_init;};
					//_vehCrew = _veh select 0;					
					//{_x setVariable ["asr_ai_exclude",true];} foreach _vehCrew;
