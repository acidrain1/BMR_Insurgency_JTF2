...
BTC_set_gear =
{
	...
	/*
	//Jig adding
	if (count (_gear select 12) > 0) then 
	{
		//{_unit addItemToUniform _x} foreach _uniform_items;
		for "_i" from 0 to (count (_gear select 12) - 1) do
		{
			{_unit addItemToUniform _x} foreach (_gear select 12);
		};
	};
	*/	
	if (count ((_gear select 6) select 0) > 0) then 
	{
		for "_i" from 0 to (count ((_gear select 6) select 0) - 1) do
		{
			...
			(unitBackpack _unit) addItemToBackpack [((_gear select 6) select 0) select _i,((_gear select 6) select 1) select _i];//Jig substituting, fixes backpack loadout but uniform inventory now botched.
		};
	};	
...
BTC_check_action_drag =
{
	...
	if (count _men > 1) then
	{
		if (BTC_pvp == 1) then
		{
			if (side (_men select 1) == BTC_side) then
			{		
				if (format ["%1", (_men select 1) getVariable "BTC_need_revive"] == "1" && !BTC_dragging && format ["%1", (_men select 1) getVariable "BTC_dragged"] != "1") then {_cond = true;};
			};
		}
...
BTC_vehicle_mobile_respawn =
{
	...
	switch (true) do
	{	
		case (_var == "MHQ_1") : {_VarName = "MHQ_1"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; [[_veh,_var],"mhq_actions_fnc",WEST] spawn BIS_fnc_MP;};
		case (_var == "MHQ_2") : {_VarName = "MHQ_2"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; [[_veh,_var],"mhq_actions_fnc",WEST] spawn BIS_fnc_MP;};
		case (_var == "MHQ_3") : {_VarName = "MHQ_3"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; [[_veh,_var],"mhq_actions_fnc",WEST] spawn BIS_fnc_MP; [_veh] call paint_heli_fnc;};
		case (_var == "Opfor_MHQ") : {_VarName = "Opfor_MHQ"; _veh setVehicleVarName _VarName; _veh Call Compile Format ["%1=_This ; PublicVariable ""%1""",_VarName]; [[_veh,_var],"mhq_actions_fnc",EAST] spawn BIS_fnc_MP;};		
	};
};
BTC_out_of_lifes =
{
