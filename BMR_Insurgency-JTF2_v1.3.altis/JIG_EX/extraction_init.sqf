/*
 extraction_init.sqf v1.1 by Jigsor
...
JIG_EX_Chopper_Type	= 	["B_Heli_Transport_01_F"];		// Desired Evac Chopper classname in quotes and array.// WIP Testing to Support: "B_Heli_Transport_01_camo_F"	"CH49_Mohawk_FG"
JIG_EX_Side			=	WEST;							// Side or Group Chopper belongs to, ie. RESISTANCE EAST WEST EvacGroup
JIG_EX_Chopper_size = 	14;								// 12 meters is good for GhostHawk.//If to small chooper cannot land. If to large, clear enough area will not be found.
...
JIG_EX_Despawn_Time	= 	45;								// Delete Chopper timer. Starts after choper reaches destination and caller's group dissembarks. Action does not return until this completes or choper destroyed.
JIG_EX_damage	=		false;							// true allows damage to be taken / false prohibits damage to be taken.
JIG_EX_Group_Dis	=	500;							// (WIP) Distance used to inventory group members of Evac caller within this range of Evac caller.//Choper will not Leave Evac LZ untill all human members in this range are in chopper (or chopper already filled with members, needs testing...).
...
{
	[] spawn {
		EvacHeliW1 = ObjNull;// required for reset
	...
	if (evac_toggle) then
	{
		...
		if (!isDedicated) then
		{
			if (!(isNull player)) then
			{
				...
				if ((local player) and (JIG_EX_Caller == player)) then
				{
					private "_ex_caller";
					jig_ex_actid_show = 9999;
					_ex_caller = JIG_EX_Caller;
...
// Modded HAFM helis @hafm_helis: UH60M "UH60M", UH60M_MEV "UH60M_MEV", CH47F "CH_47F".
// Modded Blufor Wildcat @NATO_HELCAT: AW159 Armed Camo "AW159_Armed_Camo", AW-159 Transport Camo "AW159_Transport_Camo", AW-159 Transport Black "AW159_Transport_Black", AW159 Transport Grey "AW159_Transport_Grey"
// Modded Boeing/SOAR MH-47E: "kyo_MH47E_base", "kyo_MH47E_HC", "kyo_MH47E_Ramp" Co-pilot is Gunner01. Left Gunner is Gunner02. Right Gunner is Gunner03. Loadmaster is Gunner04. Backcrew is Gunner05. //"kyo_MH47E_base" not supported
// Modded Blufor Mmohawk @KDK_Mohawk;"CH49_Mohawk_FG"
*/