								...
								if (_aMin > 0) then {
										...
										if (!surfaceiswater _mPos) then {
											0=[_mPos,units _aGroup,_mkrX,0,[0,20],true,true] call shk_fnc_fillhouse_civ;
												...
												0=[_aGroup,"INFskill"] call eos_fnc_civ_grouphandlers;
					...
					if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
							...
							if (_bMin > 0) then {	
									...
									_bGroup=[_pos,_bSize,_faction,_side] call eos_fnc_spawngroup_civ;
										...
										0=[_bGroup,"INFskill"] call eos_fnc_civ_grouphandlers;
		...
		_newpos=[_mkr,50] call eos_fnc_findsafepos_civ;
			if (surfaceiswater _newpos) then {_vehType=8;_cargoType=10;}else{_vehType=7;_cargoType=9;};
				...
				if ((_cSize select 0) > 0) then{
						0=[(_cGroup select 0),_cSize,(_cGroup select 2),_faction,_cargoType] call eos_fnc_setcargo_civ;
						};
						
							0=[(_cGroup select 2),"LIGskill"] call eos_fnc_civ_grouphandlers;
		...
		_newpos=[_mkr,50] call eos_fnc_findsafepos_civ;
			if (surfaceiswater _newpos) then {_vehType=8;}else{_vehType=2;};
					...
					_dGroup=[_newpos,_side,_faction,_vehType]call EOS_fnc_spawcivnvehicle;
						...
						0=[(_dGroup select 2),"ARMskill"] call eos_fnc_civ_grouphandlers;
	...
	for "_counter" from 1 to _eGrps do {
		...
		_newpos=[_mkr,50] call eos_fnc_findsafepos_civ;
					...
					_eGroup=[_newpos,_side,_faction,5]call EOS_fnc_spawcivnvehicle;
						...
						0=[(_eGroup select 2),"STAskill"] call eos_fnc_civ_grouphandlers;
...
if ((_fSize select 0) > 0) then {
	_cargoGrp = createGroup _side;
		0=[(_fGroup select 0),_fSize,_cargoGrp,_faction,9] call eos_fnc_setcargo_civ;
			0=[_cargoGrp,"INFskill"] call eos_fnc_civ_grouphandlers;
		_fGroup set [count _fGroup,_cargoGrp];
			null = [_mkr,_fGroup,_counter] execvm "eos_civ\functions\CivTransportUnload_fnc.sqf";
					...
					_wp1 setWaypointType "SAD";};
						...
						0=[(_fGroup select 2),"AIRskill"] call eos_fnc_civ_grouphandlers;
			
if (_debug) then {player sidechat format ["Chopper:%1",_counter];0= [_mkr,_counter,"Chopper",(getpos leader (_fGroup select 2))] call EOS_civ_debug};
			};	
