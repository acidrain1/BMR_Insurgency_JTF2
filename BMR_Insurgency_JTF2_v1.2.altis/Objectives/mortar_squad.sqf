...
{_x addeventhandler ["killed","(_this select 0) spawn {[_this] call remove_carcass_fnc}"];} forEach (units mortar_grp);
...
_tasktopicW = "Destroy Mortar Squad";
_taskdescW = "Destroy Mortar Squad";
[_tskW,_tasktopicW,_taskdescW,WEST,[],"created",_newZone] call SHK_Taskmaster_add;
sleep 5;

// create east task
_tskE = "tskE_defend_mortars" + _rnum;
_tasktopicE = "Defend Mortar Squad";
_taskdescE = "Defend Mortar Squad";
[_tskE,_tasktopicE,_taskdescE,EAST,[],"created",_newZone] call SHK_Taskmaster_add;

while {_run} do
{
	//if ({alive _x and !captive _x} count units mortar_grp > 0) then
