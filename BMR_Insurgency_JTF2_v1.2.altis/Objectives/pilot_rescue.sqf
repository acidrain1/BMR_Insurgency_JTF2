...
_pilot addeventhandler ["killed","(_this select 0) spawn {[_this] call remove_carcass_fnc}"];
...
_tasktopicW = "Rescue Pilot";
_taskdescW =  "Rescue the downed pilot and return him to base.";
[_tskW,_tasktopicW,_taskdescW,WEST,[],"created",_newZone] call SHK_Taskmaster_add;
sleep 5;

// create east task
_tskE = "tskE_Pilot_Rescue" + _rnum;
_tasktopicE = "Sabatoge Pilot Rescue";
_taskdescE =  "Sabatoge pilot rescue by killing him befor Blufor can rescue him.";
...
for [{_loop=0}, {_loop<1}, {_loop=_loop}] do
{
	...
	_nearUnits = nearestObjects [(getPos _pilot),["CAManBase"],5];
	_nearUnits = _nearUnits - [_pilot];
	{
