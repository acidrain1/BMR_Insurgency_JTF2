...
if(!_isRepetitive) then {// old method using selectWeapon, cycles and tries to selectWeapon all assigned items
	...
	if(_weaponHasChanged) then {// select back originaly selected weapon and mode, only if weapon has changed
		if(_isOnFoot) then {
...
if(count _this < 4) then {// addAction support
	_data;
}else{  
	loadout = _data;
