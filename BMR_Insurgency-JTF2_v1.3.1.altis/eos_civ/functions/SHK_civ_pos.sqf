/*
  SHK_civ_pos - Random position generator for Arma 2
  ...
  Usage examples:
    myPos = ["markerName"] execvm "SHK_civ_pos.sqf";
    myPos = ["myMarker1",true] execvm "SHK_civ_pos.sqf";
  ...
  Example of creating multiple positions:
    SHK_civ_pos = compile preprocessfile "SHK_civ_pos.sqf";
    for "_i" from 0 to 500 do {
      _p = [getpos player,random 360,random 1000] call SHK_civ_pos;
      if (count _p > 0) then {
        call compile format ["
