/*
 extraction_fncs.sqf v1.11 by Jigsor
...
extraction_pos_fnc = {
	...
	{
		tempExtMarker = [];
		if (!isNil "tempExtMarker") then {deleteMarker "tempExtMarker";};
...
drop_off_pos_fnc = {
	...
	private ["_posnotfound","_counter","_dis","_cooX","_cooY","_wheX","_wheY","_DropOffRandomPos","_newPos","_tempmkr1","_veh","_VarLZName"];
	_posnotfound = [];
	_counter = 0;
	_dis = JIG_EX_Clear_Pos_Dis;
	...
	if (!(_newPos isEqualTo [])) then
	{
		...
		_tempmkr1 = createMarker ["tempDropMkr", _newPos];
		_tempmkr1 setMarkerShape "ELLIPSE";
...
Evac_Spawn_Loc = {
	...
	"EvacSpawnMkr" setMarkerType "Empty";//set marker type to "mil_dot" for debug. Set "Empty" for invisible
...
Ex_LZ_smoke_fnc = {
	...
	["Smoke out at Evac LZ", "JIG_EX_MPhint_fnc"] call BIS_fnc_mp;
...
animate_doors_fnc = {
	...
	switch (true) do {
		...
		default {};
		//case ((_veh isKindOf "B_Heli_Transport_03_F") || (_veh isKindOf "B_Heli_Transport_03_unarmed_F")): {if ((_veh doorPhase "Door_rear_source") == 0) then {{_veh animateDoor [_x, 1]} forEach ["door_L","door_R"];} else {{_veh animateDoor [_x, 0]} forEach ["door_L","door_R"];}};
